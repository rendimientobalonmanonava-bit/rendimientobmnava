...
| Camino a Champions — estructura | Dos partes: lo **ya conseguido** en una tira de etiquetas doradas (una línea, no una fila de bloques) y lo que **falta** en la rejilla, ordenado de más cerca a más lejos. Cada prueba lleva el **cambio desde la valoración anterior de esta temporada**, con el signo real del número y el color diciendo si es a mejor o a peor. *No se usan flechas*: en las pruebas de menos-es-mejor (grasa, asimetrías, sprints) una flecha verde hacia arriba junto a un 9,3 % se lee como que ha subido. Ambas mediciones tienen que ser posteriores a `TEMP_INICIO`. *Decisión de Daniel el 21/09/2026* |
